...
Subscribt AI is a policy analysis platform that enables users to query uploaded policy documents using AI. The platform translates dense legal language into actionable guidance while maintaining 100% fidelity to source material through strict grounding and verifiable citations.
...
- **Chat_Service**: The Lambda Function URL endpoint that handles streaming AI responses using RAG and Chain-of-Thought prompting with Claude 4.6 Sonnet
...
- **Analytics_Aggregator**: Lambda function triggered by DynamoDB Streams that computes aggregated query trend data for analytics dashboards
- **User**: Any user of the platform with access to all features (document upload, query, analytics)
...
- **Query_Log**: Record of a user query stored in Metadata_Store
...
**User Story:** As a User, I want to upload Policy_Documents, so that I can query them for policy guidance.

#### Acceptance Criteria

1. WHEN a User uploads a PDF file, THE System SHALL store the file in S3 with a unique identifier
...
5. THE System SHALL store document metadata (filename, upload timestamp, user identifier, document status) in Metadata_Store

### Requirement 2: Document OCR Processing

**User Story:** As a User, I want uploaded Policy_Documents to be automatically processed, so that their content becomes queryable.
...
### Requirement 5: Query Interface

**User Story:** As a User, I want to ask questions about Policy_Documents in plain English, so that I can understand rights and obligations without reading dense legal text.

#### Acceptance Criteria

1. THE System SHALL provide a text input interface for Users to submit natural language queries
2. WHEN a User submits a query, THE System SHALL call the Chat_Service Lambda Function URL
...
**User Story:** As a User, I want AI responses grounded in uploaded Policy_Documents, so that I receive accurate policy guidance without hallucinations.
...
4. THE RAG_Engine SHALL invoke Claude 4.6 Sonnet via Bedrock with the constructed prompt
...
**User Story:** As a User, I want to know when my question is not addressed in the Policy_Documents, so that I do not receive hallucinated or inferred information.
...
**User Story:** As a User, I want every AI response to include Citations, so that I can verify the information against the source Policy_Document.
...
### Requirement 9: Query Logging

**User Story:** As a User, I want my queries logged for analytics, so that I can view trend analysis and identify common topics.

#### Acceptance Criteria

1. WHEN a User submits a query, THE System SHALL store a Query_Log in Metadata_Store
2. THE Query_Log SHALL include query text, timestamp, document identifiers, and user identifier
3. WHEN a Query_Log is written to Metadata_Store, THE System SHALL trigger a DynamoDB Stream event
4. THE System SHALL retain Query_Logs for 90 days before automatic deletion
5. THE System SHALL emit structured logs to CloudWatch for all query operations

### Requirement 10: Analytics Aggregation

**User Story:** As a User, I want to see aggregated Trend_Data on queries, so that I can identify common concerns and policy gaps.
...
**User Story:** As a User, I want to view Trend_Data on a dashboard, so that I can understand which policy topics are most frequently queried.

#### Acceptance Criteria

1. THE System SHALL provide a dashboard displaying Trend_Data from the aggregates table
...
5. THE dashboard SHALL provide export functionality for trend data

### Requirement 12: Gap Analysis

**User Story:** As a User, I want to identify topics not covered by current Policy_Documents, so that I can address policy gaps.

#### Acceptance Criteria

1. WHEN a query returns "Not addressed in the provided policy," THE System SHALL flag the query topic for Gap_Analysis
2. THE System SHALL aggregate flagged query topics in the Gap_Analysis section of the dashboard
...
5. THE System SHALL allow Users to mark gap topics as "addressed" after uploading new Policy_Documents

### Requirement 13: Document Management

**User Story:** As a User, I want to view and manage uploaded Policy_Documents, so that I can keep policy content current.

#### Acceptance Criteria

1. THE System SHALL provide an interface listing all uploaded Policy_Documents with metadata
2. THE interface SHALL display document status (processing, ready, failed) for each Policy_Document
3. THE System SHALL allow Users to delete Policy_Documents
4. WHEN a User deletes a Policy_Document, THE System SHALL remove the file from S3, delete associated Document_Chunks from Vector_Store, and update Metadata_Store
5. THE System SHALL display processing errors for failed Policy_Documents with actionable error messages

### Requirement 14: Unified User Interface

**User Story:** As a User, I want a unified interface with access to all features, so that I can upload documents, query them, and view analytics in one place.

#### Acceptance Criteria

1. THE System SHALL provide a single unified interface for all users
2. THE System SHALL provide navigation to upload, query, and analytics features
3. THE System SHALL display consistent layouts and navigation across all features
4. THE System SHALL allow seamless switching between upload, query, and analytics views
5. THE System SHALL persist user preferences and session state across feature views
...
2. THE logs SHALL include request identifiers, user identifiers, timestamps, and operation outcomes
...
3. WHEN document processing fails, THE System SHALL notify the user with the failure reason
...
3. THE System SHALL organize feature-specific components by feature (dashboard, upload, query)
4. THE System SHALL maintain consistent styling and component patterns across all features
5. THE System SHALL use Tailwind CSS for all custom styling
